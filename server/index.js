...
app.post('/api/analyze_document', requireAuth, async (req, res) => {
  ...
  const SYSTEM = 'You extract structured financial data from ANY financial document: bank statements, credit-card statements, brokerage/IRA/401k investment statements, insurance statements, and bills. First identify the document type, then fill the matching fields. Return ONLY valid minified JSON, no prose, no markdown fences. Schema: {"docType":"bank_statement|credit_card|investment|insurance|bill|receipt|other","institution":string|null,"accountName":string|null,"accountMask":string|null,"statementBalance":number|null,"accountValue":number|null,"amountDue":number|null,"premium":number|null,"dueDate":"YYYY-MM-DD"|null,"apr":number|null,"creditLimit":number|null,"minimumPayment":number|null,"transactions":[{"date":"YYYY-MM-DD","description":string,"amount":number}],"holdings":[{"name":string,"value":number}]}. Rules by type: bank_statement/credit_card -> fill statementBalance, transactions (amount sign: POSITIVE = charge/purchase money out, NEGATIVE = payment/credit/refund money in), and for credit cards also amountDue, apr, creditLimit, minimumPayment. investment (brokerage/IRA/401k/retirement) -> fill accountValue with the TOTAL ending account value and holdings with each position name + market value; transactions can be []. insurance -> fill premium (the amount owed) and dueDate; transactions []. bill -> fill amountDue and dueDate. Use null for unknown fields, dates as YYYY-MM-DD, numbers with no currency symbols or commas. If the document is unreadable, return {"error":"reason"}.';
  try {
    let userContent;
