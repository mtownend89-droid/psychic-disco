...
function _applyThemeSkin(themeId){
  ...
  const c=sk.controls||{};
  const fill=(sel,ctl)=>ctl?(p+sel+'{background:url("'+ctl.url+'") center/100% 100% no-repeat!important;border:none!important;box-shadow:none!important;'+(ctl.text?'color:'+ctl.text+'!important;':'')+'}'):'';
  css+=fill('.btn',c.btn);
  css+=fill('.btn.primary',c.btnPrimary);
  css+=fill('.btn.danger-btn',c.btnDanger);
  css+=fill('.manual-add-btn',c.btn);
  if(c.toggleOff) css+=p+'.toggle{background:url("'+c.toggleOff.url+'") center/100% 100% no-repeat!important;border:none!important;}';
  if(c.toggleOn) css+=p+'.toggle.on{background:url("'+c.toggleOn.url+'") center/100% 100% no-repeat!important;}';
  if(c.toggleOff||c.toggleOn) css+=p+'.toggle .toggle-knob{display:none!important;}';
  if(c.input) css+=p+'.mf-in{background:url("'+c.input.url+'") center/100% 100% no-repeat!important;border:none!important;color:var(--text)!important;}';
  el.textContent=css;
}
let _fxLayer=null;
